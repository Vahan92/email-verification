...
const Token = require('../models/token-verification');
const app = express();
...
* POST /signup
*/
router.signupPost = function(req, res, next) {
...
* POST /confirmation
*/
router.confirmationPost = function (req, res, next) {
    req.assert('email', 'Email is not valid').isEmail();
    req.assert('email', 'Email cannot be blank').notEmpty();
...
router.loginPost = function(req, res, next) {
    req.assert('email', 'Email is not valid').isEmail();
    req.assert('email', 'Email cannot be blank').notEmpty();
...
* POST /resend
*/
router.resendTokenPost = function (req, res, next) {
    req.assert('email', 'Email is not valid').isEmail();
    req.assert('email', 'Email cannot be blank').notEmpty();
    req.sanitize('email').normalizeEmail({ remove_dots: false });
...
generateToken = (user) => {
    return new Token({ _userId: user._id, token: crypto.randomBytes(16).toString('hex') });
}
